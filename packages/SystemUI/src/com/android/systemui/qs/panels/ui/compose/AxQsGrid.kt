...
fun axQsGridCellWidth(gridWidth: Dp, columns: Int, spacing: Dp): Dp =
...
fun axQsTileGridPageCount(itemCount: Int, columns: Int, rows: Int): Int {
    if (itemCount == 0 || rows == 0) return 1
    return ((itemCount + columns * rows - 1) / (columns * rows)).coerceAtLeast(1)
}

val AX_TILE_LABEL_HEIGHT = 24.dp

fun useAxQsCircleCells(
    gridWidth: Dp,
    tileColumns: Int,
    spacing: Dp,
...
fun axQsTileIconSize(tileSize: Dp): Dp =
    AxTileDefaults.IconSize * (tileSize / AxTileDefaults.TileHeight).coerceAtLeast(1f)

const val AX_QS_CONTROL_MAX_ROWS = 6
const val AX_QS_TILE_MAX_ROWS = 3
const val AX_QS_SHARED_MAX_ROWS = 6

val AX_TILE_MAX_SIZE = 85.dp
